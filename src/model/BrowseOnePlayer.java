...
public class BrowseOnePlayer {
	...
	public ArrayList<Player> selectOnePlayerList(int PlayerId) {
        ...
        DB db=new DB();  
        String sql="SELECT comments, commentid, comments.userid userid, player.PLAYERID playerid, playername, avg(RATINGS) ratings, goalscored, teamname, category, height, dob, PICURL, clubname, location FROM player,Team, PlaysFor P, comments, rate, PlayerPic, position, playsat, Club, belongsto where p.TeamID=Team.TeamID and p.PlayerID=Player.PlayerID and playerpic.playerid=Player.PlayerID and comments.playerid = Player.PlayerID and rate.playerid=Player.PlayerID and playsat.playerid=Player.PlayerID and playsat.positionid=position.positionid and belongsto.playerid=Player.PlayerID and belongsto.clubid=club.clubid and player.playerid=" + pid+ "group by commentid,comments.userid,picurl,playername, COMMENTS, player.PLAYERID, goalscored, teamname, category, height, dob, PICURL, clubname, location";
        rs = db.doSelect(sql);
        ...
        while (rs.next()) {
        	...
        	player.setCID(rs.getInt("commentid"));
